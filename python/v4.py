...
# 标定
pts_o1 = np.float32([[275.5, 28.1], [750.5, 1.9], [225.2, 445], [918, 414.5]])
...
pts_o5 = np.float32([[198.9, 77], [747.1, 87.1], [185.8, 490.4], [745.7, 502.4]])
...
#pts_o1 = np.float32([[275.5, 28.1], [750.5, 1.9], [225.2, 445]])
#pts_o2 = np.float32([[136.5, 78.4], [688.2, 77.7], [135.8, 506.7]])
#pts_o3 = np.float32([[184, 75.6], [734.3, 90.5], [167.7, 499.6]])

#pts_o4 = np.float32([[230.1, 57.2], [787.5, 32.3], [240.8, 471.3]])
#pts_o5 = np.float32([[198.9, 77], [747.1, 87.1], [185.8, 490.4]])
#pts_o6 = np.float32([[192.6, 81.3], [738.6, 98.3], [172, 508.2]])

#pts_o7 = np.float32([[206.7, 55.7], [757.7, 37.3], [211.6, 472.7]])
#pts_o8 = np.float32([[153.6, 41.6], [716.7, 60.7], [150, 473.4]])
#pts_o9 = np.float32([[121.6, 42.3], [691.8, 40.9], [132.3, 477]])
...
for i in pts_o:
    ...
    #M.append(cv2.getAffineTransform(i, pts_d))
...
#    print(time_a)
    ...
    for barcode in barcodes:
        ...
        realPosition,RotationAngle = Conter(x,y,cutImage,1920,1080,4.2,2.4)
        if realPosition[0] > -1:
...
def nine_to_one(queues):
    ...
    while True:
        ...
        for i in range(1):
            ...
            #images.append(cv2.warpAffine(queues[i].get(), M[i], (960, 540)))
